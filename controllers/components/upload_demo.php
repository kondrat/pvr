<?php	
	class UploadDemoComponent extends Object {
		...
		function image ($file,  $size, $qlt = null ) {
			...
			if ( $uploadWidth < $uploadHeight ) {
				$this->error (__('Demo must be horizontal',true) );
				return;
			} elseif( $uploadWidth < 630 ) {
				$this->error (__('Demo width must be more than 630px',true) );
				return;
			} elseif( $uploadHeight < 550 ) {
				$this->error (__('Demo height must be more than 550px',true) );
				return;
			} 
			
			$size = array('sq' => '75', 'md' => '630');
			...
			foreach( $size as $k => $v ) {
				switch($k) {					
					case('sq'):										
							...
							//to check!!!!
							imagecolorallocate($dstImg, 255, 0, 0);
					...
					default:
							...
							$srcY = round( ($uploadHeight*$newX/$uploadWidth - $newY)/2);
							//debug($srcY);
							$srcX = 0;//round( ($uploadHeight - (630 / $ratioX) )/2);
							
							
						
							$dstImg = imagecreatetruecolor($newX, $newY);
							imagecopyresampled($dstImg, $srcImg, 0, 0,$srcX, $srcY, $newX, $newY, $uploadWidth, $uploadWidth*$ratio);
